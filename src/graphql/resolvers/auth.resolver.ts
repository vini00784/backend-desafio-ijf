...
export class AuthResolver {
    ...
    async createUser(
        ...
        try {
            if(input.userRole === "teacher")
                return await this.#createTeacher(input);
    ...
    async #createTeacher(teacherData: CreateUserInput): Promise<CreateUserResponse> {
        const { id, username, createdAt } = await prisma.teacher.create({
            data: {
                name: teacherData.name,
                username: teacherData.username,
                password: await hash(teacherData.password, +this.configService.get<number>('BCRYPT_SALT_ROUNDS')),
            }
        });
    ...
    async #createStudent(studentData: CreateUserInput): Promise<CreateUserResponse> {
        const { id, username, createdAt } = await prisma.student.create({
            data: {
                name: studentData.name,
                username: studentData.username,
                password: await hash(studentData.password, +this.configService.get<number>('BCRYPT_SALT_ROUNDS')),
            }
        });
