...
export class Course extends BaseEntity<CourseProps> {
  ...
  removeLesson(lessonId: Lesson['id']) {
    ...
    const foundedLesson = this.props.lessons.findIndex(
  ...
  addStudent(student: Student) {
    if (
      this.props.students.findIndex(
        (studentItem) => studentItem.props.username === student.props.username,
      ) !== -1
    )
  ...
  removeStudent(studentId: Student['id']) {
    const foundedStudent = this.props.students.findIndex(
