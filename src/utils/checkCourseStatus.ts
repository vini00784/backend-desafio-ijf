...
export async function checkCourseStatus(
    ...
    const studentLessons = studentCourse.course.lessons.flatMap(item => item.studentLessons); // O "flatMap" resolve o problema de retornar um "array de arrays", que estava ocorrendo com a utilização do map convencional. O flatMap mapeia um a um e achata tudo em um só array, enquanto o map cria um novo array a partir do resultado mas sem alterar a estrutura do array resultante.

    const watchedLessonsCount = studentLessons.filter(studentLesson => studentLesson.isWatched).length;
    ...
    else if(watchedLessonsCount > 0 && watchedLessonsCount < studentLessons.length)
